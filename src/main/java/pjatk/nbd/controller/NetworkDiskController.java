package pjatk.nbd.controller;

import jakarta.servlet.http.HttpServletResponse;
import pjatk.nbd.AppConfig;
import pjatk.nbd.service.NetworkDriveService;
...
public class NetworkDiskController {
    ...
    public String showFile(Model model, @PathVariable(name = "pathUrl", required = false) String pathUrl) {
        if (pathUrl != null) {
            path += "\\" + pathUrl;
        }
        ...
        model.addAttribute("disk", AppConfig.PJATK_USER_DISK_NAME);
        return "NetworkDisk";
    }

    @PostMapping("/updatePath")
    public String updatePath(@RequestParam(name = "pathUrl") String pathUrl) {
        disk = pathUrl;
